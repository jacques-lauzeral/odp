...
**Chapter `osHierarchy` referential-integrity cascade.** `osHierarchy` is a JSON blob on `ChapterVersion`, not graph edges — Neo4j cannot enforce referential integrity against it. Two O\* writes take an O\* out of a domain chapter's scope and would otherwise leave a dangling `osHierarchy` ref: a **soft delete** (the O\* leaves every scope) and a **domain change** on update/patch (the O\* leaves its former domain's scope). The base wires both triggers into a single seam, `_detachFromChapterOsHierarchy(itemId, domain, changeSetCommit, tx)`, run inside the triggering transaction so the chapter excise and the O\* write commit atomically:

- `softDelete` calls it after the store transition with the item's **current** domain.
- `_doUpdate` (update) and `patch` call it via `_cascadeDomainChange(...)` with the **former** domain when the write changed `domain`; the new chapter needs no write — the O\* simply reappears there as an unclassified item.

The seam defaults to a no-op; a service opts in via `_cascadesDomainChange()` (true on `OperationalRequirementService` / `OperationalChangeService`, false elsewhere — and `ChapterService` keeps the hook no-op, a chapter write never cascades to another chapter). Opt-in also gates the one extra pre-write read `_doUpdate` needs to learn the former domain (`patch` already holds `current`, so it reads nothing more). The override delegates the hierarchy mutation to `ChapterService.exciseOStarFromChapter`, which writes a **new** `ChapterVersion` (the append-only model is preserved — any version already captured by an edition is untouched) reusing the triggering operation's `changeSetCommit`, so the chapter edit is audited under the same change set as the O\* event that caused it.
...
- `getById(itemId, user, editionId?, projection?, lifecycleFace?)` — defaults to `'extended'` projection; merges config-owned fields (including `availableBlockIds` from `edition.json`) and enriches `osHierarchy` items with `{id, type, code, title}` objects resolved from the **domain-scoped** O\* set via `_buildOStarMap(user, domain, editionId)`. Refs not in that set — an O\* that was soft-deleted, or moved to another domain — are **dropped on read** (not rendered as broken placeholders) and the count + ids logged once per chapter at WARN. The filter runs in every context, intersecting against the O\* set resolved in the **same** context (live or edition), so an edition view matches what the user saw at capture time; any resulting edition skew is a data-quality concern surfaced at edition-creation time, not a read-path branch. `lifecycleFace` is carried for base-signature alignment and passed through (chapters expose only the `active` face).
- `_buildOStarMap(user, domain, editionId?)` — delegates to `OperationalRequirementService.getAll()` and `OperationalChangeService.getAll()` (both `'summary'` projection, filtered to `{ domain }`, in the given context) so transaction lifecycle is owned by the service layer. Returns a `Map<normalizedItemId, {id, type, code, title}>` — the domain O\* set the read filter intersects `osHierarchy` against.
- `exciseOStarFromChapter(itemId, domain, changeSetCommit, tx)` — the write-path cascade target (called by O\* services via `_detachFromChapterOsHierarchy`). Resolves the chapter owning `domain` from `getChapters()` config, loads it fresh in the transaction, strips every reference to `itemId` from the `osHierarchy` tree (topics retained even if emptied — theme removal is a separate user action), and writes a new `ChapterVersion` via `store.update` **only when the hierarchy changed**, reusing the triggering `changeSetCommit`. `_detachFromChapterOsHierarchy` is overridden to a no-op on `ChapterService` itself (chapters do not cascade to chapters).
