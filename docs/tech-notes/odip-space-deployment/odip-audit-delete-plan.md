...
> **Status (18 Jun 2026):** §4.1 model/shared ✅ DONE · §4.2 storage ✅ DONE · §4.3 service ✅ DONE · §4.4 REST ✅ DONE · §4.5 CLI ✅ DONE · §4.6 web ✅ DONE · §4.7 ADD (01 ✅, 02 ✅, 03 ✅, 04 ✅, 07 ✅, 08 ✅) — **Phase B in-round scope complete.** §4.1/§4.2 were briefly reopened to correct `OperationalEntityReference` scope (O\* only) and `findInboundReferences` (O\* where-used only; edition wall removed, relocated to DEL-04 hard delete) — correction applied across code, ADD 01/02, and design note. §4.4 settled the `409` refusal contract as `LifecycleConflictResponse` (`references` a top-level sibling of `error`) and recorded that `DELETE /:id` persists as the whole-item hard destroy alongside the new `POST /:id/delete` soft delete — design note §4.4 and §5 matrix updated accordingly. §4.5 added a user-initiated `inbound-references` query verb (distinct from the non-preemptive `delete`) — design note §4.5 updated to record the distinction. §4.6 built soft-delete-from-the-detail-form across all three O\* detail contexts (collection panel, full page, narrative panel) via a shared `os-delete.js` flow, the `confirmLabel`/`dangerConfirm` commit-dialog options, and the `odipLifecycleConflict` dialog; ADD 08 updated in the same batch. **Carried out of scope:** the NarrativeActivity post-delete cleanup of the chapter `osHierarchy` — a soft-deleted O\* leaves the live dataset but may still be referenced by a chapter's hierarchy. This is **not new to DEL** (the same impact is triggered by an O\* domain change) and is tracked as its own design question (§6), not a DEL gap.
...
**In-round scope.** Lifecycle-edge model; soft delete + restore (per-item); referential integrity (live O\* inbound-reference guard, DEL-01); the released-state soft-delete guard (DEL-02a — the published-item gate as the `released` lifecycle state, not edition membership); the `lifecycleFace` read model; strict-payload rejection. **Deferred** (designed, not built): release (DEL-06a, P1) / decommission (DEL-06b, P1), hard delete (DEL-04, P1) and its edition wall (DEL-02b, P1), edition deletion (DEL-05, P1), `BatchService`, and the non-soft-delete web client. All P0 requirements (LCM-03, DEL-01, DEL-02a) are met by the in-round scope; everything deferred is P1.
...
> - `--lifecycle-face` (renamed from the sketch's `--lifecycle-status`, to match the `lifecycleFace` parameter at every other layer and avoid colliding with the `lifecycleStatus` read-shape) on **both** `list` and `show`, validated and checked for edition/baseline exclusivity by a shared `resolveLifecycleFace` helper; forwarded as `?lifecycleFace=` only when non-default (mirrors the `projection` pattern). On `show` it is the only way to inspect a soft-deleted item by id (the REST single-item read already accepts the param). Single `Lifecycle` column via `formatLifecycleStatus` (lists the true flags), and the same label on `show` via the base `displayItemDetails`.
...
> - **Service-layer fixes (found in testing, now resolved in `VersionedItemService.js`):** (1) `softDelete`/`restore` re-read the item on its post-transition face and return the full shape (was thin — `title: undefined` / `Lifecycle: —`). (2) `getInboundReferences` gained a `_findOnAnyFace` existence guard — absent id → `null` → 404, deleted target still resolves. (3) latent: `softDelete`/`restore` now split nonexistent (404) from invalid-state (409), fixing a missing-id 500. The inbound-references route maps the service `null` to 404.
...
- **Soft-delete/restore response shape** — ✅ resolved: `softDelete` / `restore` now re-read the item on its post-transition face (`deleted` after soft delete, `active` after restore) and return that full shape, so the response carries `title` and `lifecycleStatus`. (Was: thin store-transition result → `title: undefined` / `Lifecycle: —`.)
- **`getInboundReferences` target-existence guard** — ✅ resolved: a new `_findOnAnyFace(itemId, tx)` helper (probes `active` then `deleted`) gates the where-used read; a truly-absent id returns `null` → 404 at the route, while a soft-deleted target still resolves and returns its live inbound list. (Was: `200` + empty list for absent/deleted ids.) **Forward note:** `_findOnAnyFace` probes only `active`/`deleted` — the faces a transition can produce this round. When DEL-06 makes `released`/`decommissioned` operable, those faces must be added, or an item in them would wrongly 404 on inbound-references.
- **Soft-delete/restore not-found vs invalid-state** — ✅ resolved (latent bug found while fixing the above): both now distinguish a nonexistent id (→ `null` → 404) from an item that exists on another face (→ `INVALID_LIFECYCLE_STATE` → 409). Previously `softDelete` threw a plain `Error('Entity not found')` that no route arm matched, so a delete on a missing id would have returned 500.
- **Release & decommission (DEL-06a / DEL-06b)** — parked, both P1; `DECOMMISSION` and `RELEASE` actions reserved so the audit shape is stable when they land. Both are integrator-assisted, never date- or OC-state-automated: the system derives release/decommission candidate lists and the integrator confirms explicitly per O\*, batch-applied via `applyLifecycleBatch`. Structurally identical (shared worksheet + batch). A separate note will cover candidate derivation and the worksheet UX. (Supersedes the earlier single-DEL-06, P0-starred framing — see design note §5.6.)
...
- **O\* transaction impact on chapter `osHierarchy`** — **open, broader than DEL, carried out of scope.** A chapter's `osHierarchy` references O\* items by id. When an O\* leaves the live dataset (soft delete) or changes the set of chapters that should reference it (domain change), the hierarchies that point at it become stale. This is **not introduced by DEL** — the domain-change path already triggers it — so it is not a DEL completion gap. The web layer this round clears the body and refreshes the TOC view, but does not reconcile the persisted `osHierarchy`. A dedicated design question: where does hierarchy reconciliation belong (service-side on the O\* write, a lazy read-time filter, or an explicit editorial action), and is it symmetric across delete / domain-change / restore. To be taken up separately.
...
| CLI | `audit-event` command | delete/restore/inbound-references verbs, `--lifecycle-face` on `list` + `show`, Lifecycle column + `show` label |
| web | History view (client-built over audit query) | soft-delete from O\* detail form (all 3 contexts) via shared `os-delete.js`; `confirmLabel`/`dangerConfirm` dialog opts; `odipLifecycleConflict`; `softDeleteOStar`. Restore / recycle bin / `lifecycleFace` selector P1+. `osHierarchy` reconciliation carried out of scope (§6) |
| ADD | 6 chapters (01–04 done, 07/08 pending) | 01 ✅, 02 ✅, 03 ✅, 04 ✅, 07 ✅, 08 ✅ |
