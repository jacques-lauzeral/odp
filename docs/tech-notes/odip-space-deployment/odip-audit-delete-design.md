...
| **DEL-02** — Published-edition wall | P0 | n/a (folds into the blocking-dependency check) | ✅ implemented (§5.1) |
...
**Lifecycle dataset on `list`** — `requirement list` (and `change list`) gains a `--lifecycle-status active|released|decommissioned|deleted` option (default `active`), selecting the dataset face, same other filter flags. There are no separate `deleted` / `released` / `decommissioned` verbs — `--lifecycle-status deleted` is the recycle bin, `released` is what's in production, and so on. Mutually exclusive with the edition option.

**Lifecycle state in `list` and `show`.** Both carry the four flags via the read projection (§4.1). `show <id>` displays the item's full lifecycle state — the derived state label (Active, Active + Released, Decommissioned, Deleted) plus, where relevant, the version each lifecycle edge points at (e.g. the released version may differ from the working `LATEST_VERSION`). In `list`, the displayed lifecycle information depends on the selected `--lifecycle-status`:
...
**Non-preemptive (per §4.3).** The CLI does not pre-check deletability — it calls the operation and renders the result; on a `409` refusal it prints the blocking-dependency list (and any lifecycle-state failure) as a table. Blockers surface from the refusal, not ahead of it; there is no separate `blocking-dependencies` query verb.
