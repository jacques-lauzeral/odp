...
/**
 * CRISIS_Mapper - Maps CRISIS Operational Needs and Requirements Word documents
 *
 ...
 * -----------------------------------
 * The CRISIS documents uses a simple 3-level hierarchical structure:
 *
 * - Level 1: "Crisis DrG" (document root - excluded from path)
 * - Level 2: Organizational folders (e.g., "Crisis_Information_Portal ON - OR")
 * - Level 3: Entity sections - "Operational Need (ON)" or "Operational Requirement (OR)"
 *
 * Path Construction:
 * ------------------
 * - Path = Level 2 organizational folder only
 * - Example: ["Crisis ON - OR - OC"]
 * - Level 1 root excluded, Level 3 entity marker excluded
 *
 * External ID Generation:
 * -----------------------
 * - Built from path + normalized title using ExternalIdBuilder
 * - Format: {type}:crisis/{path}/{normalized_title}
 * - Example: or:crisis/crisis_information_portal_on_or/manage_content_and_users
 *
 * Title Extraction:
 * -----------------
 ...
 * - Format in privateNotes: "ON ID: ON_CRISIS_001" or "OR ID: OR_CRISIS_001_G"
 *
 * Table Field Mapping:
 * --------------------
 ...
 * - Use Cases: Not present in CRISIS documents
 */
class CRISIS_Mapper extends Mapper {
    ...
    _processSection(section, context, ancestorPath = []) {
        // Skip level 1 root section ("Crisis DrG")
        if (section.level === 1) {
